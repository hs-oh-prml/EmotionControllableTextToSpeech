...
class AudioDataset(Dataset):
    ...
    def __init__(self,
        ...
        """
        ### for audio book ###
        # f = open('/home/prml/jihyun/raw/audiobook_k_alignscript/speakers_id.txt', 'r', encoding='UTF8')
        #
        # spid = []
        # spname = []
        # for line in f.readlines():
        #     id = line[:-1].split("_")[0]
        #     name = line[:-1].split("_")[-1]
        #     spid.append(int(id))
        #     spname.append(name)
        # f.close()
        ...
        with codecs.open(os.path.join(root_dir, csv_file), 'r', 'utf-8') as f:
            for i, line in enumerate(f.readlines()):
                # audiobook
                # wav_path, spk_name, text = line[:-1].split("|")
                # spk_id = spid[spname.index(spk_name)]

                # NIKL
                wav_path = line.split("|")[0]
                # spk_name = line[:-1].split("|")[3]
                text = line.split("|")[1]
                # spk_id = line[:-1].split("|")[2]
                # wav_name = os.path.join(root_dir, wav_path)
    ...
    def __getitem__(self, idx):
        ...
        utt_id = self.utt_ids[idx]
        # audio = self.audio_load_fn(self.audio_files[idx])
        audio = librosa.core.load(self.audio_files[idx], sr=22050)
