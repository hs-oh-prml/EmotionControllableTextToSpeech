...
def inference(a):
    ...
    filelist = sorted(glob.glob(os.path.join(a.input_wavs_dir, '*.npy')))
...
#            x = torch.from_numpy(np.load(filname)).T.unsqueeze(0).cuda()
            x = torch.from_numpy(np.load(filname)).unsqueeze(0).cuda()
...
def main():
    ...
    parser.add_argument('--input_wavs_dir', default='../mel/')
    parser.add_argument('--output_dir', default='../wav/')
    parser.add_argument('--checkpoint_file', default='./g_01800000')
    a = parser.parse_args()
