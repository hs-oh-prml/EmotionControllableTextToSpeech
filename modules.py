...
class VarianceAdaptor(nn.Module):
    ...
    def forward(self, x, src_mask, mel_mask=None, duration_target=None, pitch_target=None, energy_target=None, max_len=None):
        ...
        duration_prediction = self.duration_predictor(x, src_mask)
        ...
        if duration_target is not None:
            ...
            duration_rounded = ((duration_prediction + 0.5) * 1.0).int()
            x, mel_len = self.length_regulator(x, duration_rounded, max_len)
            mel_mask = utils.get_mask_from_lengths(mel_len)
        
        return x, duration_prediction, pitch_prediction, energy_prediction, mel_len, mel_mask
